...
import { CHEST_TIERS, isCollectible, isMedal, isShopOnly, ITEM_IDS, tierForLoot, TIER_COPY, type ChestTier, type Inventory } from './loot';
...
export function sealedRowsFromStash(inv: Inventory, stashGold: number): SealedRow[] {
  ...
  for (const id of ITEM_IDS) {
    if (!isCollectible(id) || isMedal(id) || id === 'gold-cup' || isShopOnly(id)) continue;
    const n = inv[id] ?? 0;
    if (n <= 0) continue;
