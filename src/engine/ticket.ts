...
/**
 * Hard and Campaign: a matching key only counts when socketed in `slots`.
 * Unsocketed keys stay in the pack; missing gold still blocks unless a key
 * is in a well. Campaign key never pays Hard; Hard key never pays Campaign.
 */
export function quoteEntry(
  ...
  if (mode === 'campaign') {
    const offs = normalizeOfferings(slots, meta, 'campaign');
  ...
  if (mode === 'hard') {
    const offs = normalizeOfferings(slots, meta, 'hard');
    const keyCount = Math.max(0, meta.items['hard-key'] ?? 0);
    if (hasSocketedHardKey(offs)) {
      return { mode, kind: 'key', cost, keyId: 'hard-key', keyCount, gold };
    }
    if (gold >= cost) {
      return { mode, kind: 'gold', cost, keyId: 'hard-key', keyCount, gold };
    }
    return { mode, kind: 'blocked', cost, keyId: 'hard-key', keyCount, gold };
  }
...
export function confirmCopy(quote: EntryQuote): string {
  ...
  if (quote.kind === 'blocked') {
    const need = quote.mode === 'hard' ? 'socket a Hard key' : 'socket a Campaign key';
...
export function spendEntry(
  ...
  const offs: OfferingSlots = modeUsesOfferings(mode)
    ? normalizeOfferings(slots, meta, mode)
    : [null, null];
