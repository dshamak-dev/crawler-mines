...
Audio (`public/audio/`): **cozy-descent** loops seamlessly on the title and Easy/Medium/Hard (no fade-to-silence restart, slightly quicker middle); campaign-depths on campaign floors 1–4; **flag-eater-boss** loops on the floor-5 Gluttony fight; **wrath-boss** loops when the rolled boss is Wrath. Both stay up if Collection is opened from that floor, then cozy when you return to the menu. BGM and SFX pause when the tab is hidden and resume from the same point when you return (if Sound is on). Mute persists in localStorage. SFX play on dig, flag, chest, blast, wreck, clear, UI, deny, and boss cues (move, eat flag, hit, death, campaign lose).
