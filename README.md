...
Audio (`public/audio/`): cozy BGM on the title and Easy/Medium/Hard; campaign-depths on campaign floors 1–4; **flag-eater-boss** loops on the floor-5 boss fight (Wrath reuses this track for now) until you return to the menu. BGM and SFX pause when the tab is hidden and resume from the same point when you return (if Sound is on). Mute persists in localStorage. SFX play on dig, flag, chest, blast, wreck, clear, UI, deny, and boss cues (move, eat flag, hit, death, campaign lose).
